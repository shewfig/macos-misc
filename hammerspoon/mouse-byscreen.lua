...
function doKeyMap(e)
	if tKeyHold then
		...
		-- print("doKeyMap:".. arrowKeyDir .." dx:".. dx .." dy:".. dy)
		-- print(string.format("doKeyMap: %s dx:%d dy: %d\n", arrowKeyDir, dx, dy))
		-- hs.mouse.setAbsolutePosition(oldmousepos)
...
function mouseTrackByScreen()
	screenMode = hs.screen.mainScreen():currentMode()
	hs.mouse.trackingSpeed( screenMode['w'] * screenMode['h'] / 1000000 * screenMode['scale'])
end
...
mouseScreen = hs.screen.watcher.newWithActiveScreen(mouseTrackByScreen)
mouseScreen:start()
